...
class WorkspaceData:
    def __init__(self):
         # connect to sqlite3, if there's no database.db create one, else connect to it
    ...
    def save(self, table: str, data: typing.List[typing.Tuple]):
        ...
        """
            ...
            The 1st .join() converts the list of columns to a string where each element of the column 
                is separated by a comma
            The 2nd .join() does the same as 1st .join() but use dynamically a list of ? , so the number of elements in the
                list matches the number of columns in the table
        """
