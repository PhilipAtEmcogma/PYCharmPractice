...
class StrategyEditor(tk.Frame):
    def __init__(self, root, binance:BinanceFuturesClient, bitmex:BitmexClient, *args, **kwargs):
        ...
        self._base_params = [
            ...
            {"code_name": "activation", "widget": tk.Button, "data_type": float, "text": "Off",
             "bg": "darkred", "command": self._switch_strategy, "width":8},
        ...
        for idx, h in enumerate(self._base_params):
            header = tk.Label(self._headers_frame,text="",bg=BG_COLOR,fg=FG_COLOR,font=GLOBAL_FONT, width=h['width'], bd=1, relief=tk.FLAT)
            header.grid(row=0,column=idx, padx=2)

            header = tk.Label(self._headers_frame,text="",bg=BG_COLOR,fg=FG_COLOR,font=GLOBAL_FONT, width=8, bd=1, relief=tk.FLAT)
        ...
        for h in self._base_params:
            ...
            # create additional keys for option menu
            if h['code_name'] in ["strategy_type", "contract", "timeframe"]:
                self.body_widgets[h['code_name'] + "_var"] = dict()
            # accesssing a specific element in a dict, can be done like a 2D arrary.
            #   e.g. self.body_widgets['bid'][3]
    ...
    def _add_strategy_row(self):
        ...
        for col, base_param in enumerate(self._base_params):
            ...
            if base_param['widget'] == tk.OptionMenu:
                ...
                # set each drop box to select 1st element in the list as default selected element
                ...
                self.body_widgets[code_name][b_index] = tk.OptionMenu(self._body_frame.sub_frame,
                                                                      ...
                                                                      *base_param['values'])
                self.body_widgets[code_name][b_index].config(width=base_param['width'], indicatoron=0, bd=0)
        ...
        # loops through the keys (strat) and the value (params) of the dictionary
        for strat, params in self._extra_params.items():
            # loops through the list of each key
            for param in params:
                self._additional_parameters[b_index][param['code_name']] = None # assign No value by default
    ...
    def _show_popup(self, b_index: int):
        ...
        # getting the coordinates of the button being click, so the popup window open there, instead of randomly
        ...
        # spawn the popup window next to where the button is pressed, with a few pixel offset
        self._popup_window.geometry(f"+{x-80}+{y+30}")
        self._popup_window.grab_set() # this allows user to only click on popup window untill its closed

        # to be use for storing strategy selected
        ...
        for param in self._extra_params[strat_selected]:
            ...
            # create input box for the strategy value
    ...
    def _validate_parameters(self, b_index: int):
        ...
        # close popup window after validation is complete by using the destory() function
