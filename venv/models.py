...
class Balance:
    def __init__(self, info, exchange):
        if exchange == "binance":
            ...
            self.initial_margin = info['initMargin'] * BITMEX_MULTIPLIER #return in satoshi, so need to convert to BTC
...
class Contract:
    ...
    def __init__(self, contract_info, exchange):
        if exchange == "binance":
            ...
            # price_decimals and quantity_decimals will be used to round the price
            #   and quantity of an order to a decimal accepted by Biance,
...
class OrderStatus:
    def __init__(selfself, order_info):
